	...
	$(function(){
				...
				$.ajax({
					...
					success : function (data) {
						...
						$.each(data,function (i,n) {
							html += '<tr>';
							html += '<td><input type="checkbox" name="xz" value="'+n.id+'"/></td>'; //id是市场活动的id
							html += '<td>'+n.name+'</td>';
		...
		$("#bundBtn").click(function () {
			...
			//1条或者多条
				...
				//workbench/clue/bund.do?cid=xxx&aid=xxx&aid=xxx&aid=xxx //cid线索id aid市场活动id
				var param = "cid=${c.id}&";
				...
				$.ajax({
					...
					success : function (data) {
						...
						if(data.success){
							...
							//刷新关联市场活动的列表
							showActivityList();
							//清除搜索框中的信息 复选框中的√干掉 清空activitySearchBody中的内容（未做）
							//关闭模态窗口
		...
		$.ajax({
			...
			success : function (data) {
				...
				$.each(data,function (i,n) {
					...
					html += '<td>'+n.endDate+'</td>';
					html += '<td>'+n.owner+'</td>';
					html += '<td><a href="javascript:void(0);" onclick="unbund(\''+n.id+'\')" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>'; //id是关联关系表的id
					html += '</tr>';
				})
...
<body>
	...
	<div style="position: relative; top: -70px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 70px;">
			...
			<div style="width: 630px;position: relative; left: 200px; top: -20px;"><b>${c.description}</b></div>
			<div style="height: 1px; width: 850px; background: #D5D5D5; position: relative; top: -20px;"></div>
		</div>
		<div style="position: relative; left: 40px; height: 30px; top: 80px;">
			<div style="width: 300px; color: gray;">联系纪要</div>
			<div style="width: 630px;position: relative; left: 200px; top: -20px;"><b>${c.contactSummary}</b></div>
			<div style="height: 1px; width: 850px; background: #D5D5D5; position: relative; top: -20px;"></div>
		</div>
        ...
        <div style="position: relative; left: 40px; height: 30px; top: 100px;">
            ...
            <div style="width: 630px;position: relative; left: 200px; top: -20px;"><b>${c.address}</b></div>
            <div style="height: 1px; width: 850px; background: #D5D5D5; position: relative; top: -20px;"></div>
        </div>
	</div>
